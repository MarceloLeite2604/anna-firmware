...
deploy_systemd_unit() {
    ...
    deployed_systemd_unit_file_path="${systemd_unit_files_deploy_directory_path}${unit_file_name}";
...
install_service_script() {
    ...
    local destination_service_script_path;
    local cp_result;
    local chmod_result;
...
#   3. The path to the directory where the service's scrips are (optional).
#
# Result:
...
create_service() {
    ...
    local service_name;
    local service_scripts_name;
    local service_scripts_directory_path;
    local service_script_path;
    ...
    then
        ...
        then
            ...
            service_scripts_name=("${!2}");
            service_scripts_directory_path="${3}";
        fi;
    fi;

    # If the service has shell scripts to install.
    if [ ${#service_scripts_name[@]} -ne 0 ];
    then
        ...
        do
            ...
            then
                print_error_message "Error while installing service script \"${service_script_name}\".";
                return 1;
            fi;
        done;
    fi;
    ...
    if [ ${create_systemd_unit_file_result} -ne 0 ];
    then
